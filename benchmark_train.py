...
from cs336_basics.schedule import lr_cosine_schedule
...
def main():
    ...
    parser.add_argument("--valid_data_path", type=str, default="./data/valid.bin")
    ...
    # 7. 主训练循环
    with nvtx.range("Main Train"):
        for it in range(start_iter, args.max_iters):
            # A. 更新学习率
            lr = lr_cosine_schedule(it, args.lr, args.min_lr, args.warmup_iters, args.max_iters)
            ...
            with nvtx.range("Train: forwards"):
                logits = model(x)
            ...
            with nvtx.range("Train: backwards"):
                loss.backward()
                torch.cuda.synchronize()    

            # 梯度裁剪
            clip_gradient(model.parameters(), args.max_norm)
